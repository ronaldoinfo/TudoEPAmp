<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsl-extensoes="urn:xsl-extensoes" exclude-result-prefixes="xsl-extensoes">
  ...
  <xsl:template name="cria-link-diretorio">
    ...
    <a href="{$diretorio}/{@url}" data-track-category="Notícias" data-track-event="click" data-track-label="Link: Notícia: {@manchete}">
      ...
      <div>
        <a href="{$link}" title="$manchete" data-track-category="Notícias" data-track-event="click" data-track-label="Link: Notícia: {$manchete}">
          <picture itemprop="image" itemscope="" itemtype="https://schema.org/ImageObject">
            <meta itemprop="url" content="{$nomeimagemtipo}" />
            <meta itemprop="width" content="{$larguraimagem}" />
            <meta itemprop="height" content="{$alturaimagem}" />
    ...
    <xsl:variable name="_conteudo">
      <xsl:choose>
        <xsl:when test="$caption='' and $_widthbox!='100' ">
          <picture itemprop="image" itemscope="" itemtype="https://schema.org/ImageObject">
            <meta itemprop="url" content="{$nomeimagemtipo}" />
            <meta itemprop="width" content="{$_widthbox}" />
            <meta itemprop="height" content="{$_height}" />
            <source type="image/webp" srcset="{$nomeimagemtipo}" />
    ...
    <xsl:variable name="nomeimagemtipo">
      <xsl:choose>
        <xsl:when test="$tipo!='pq'">
          ...
          <xsl:if test="$arquivo=''">
            <xsl:value-of select="https://www.acidadeon.com/img/imagem_padrao_ACidadeOn.webp"/>
          </xsl:if>
        </xsl:when>
        <xsl:when test="$tipo='pq'">
          <xsl:if test="$arquivo!=''">
    ...
    <xsl:variable name="_conteudo">
      <div style="display: none;">
        <picture itemprop="image" itemscope="" itemtype="https://schema.org/ImageObject">
          ...
          <meta itemprop="width" content="{$_widthbox}" />
          <meta itemprop="height" content="{$_height}" />
          <img src="{$nomeimagemtipo}" width="{$_widthbox}" height="{$_height}" />
        </picture>
      </div>
    </xsl:variable>
    <xsl:copy-of select="$_conteudo" />
